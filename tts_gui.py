...
CACHE_DIR = "tts_audio_cache"
CACHE_DB_PATH = "tts_audio_cache.db"
    ...
    def run(self):
        ...
        try:
                    ...
                    for line in lines:
                        ...
                        if not line:
                            ...
                            if current_timestamp and current_text:
                                ...
                                if text:
                                    all_tasks.append(
                                        (idx, current_timestamp, text, save_dir)
                                    )
                                    idx += 1
                                current_text = []
                                current_timestamp = None
            ...
            if self.config.get("use_multi_api", False):
                ...
                available_apis = [
                    api
                    for api in self.config.get("api_configs", [])
...
class TTSApp(QWidget):
        ...
        for task in tasks:
            if task["name"] == task_name:
                ...
                if invalid_files:
                    ...
                    self.log.append(
                        f"已加载任务 '{task_name}': {len(valid_files)} 个文件"
                    )
    ...
    def save_current_task(self):
        ...
        task_name, ok = QInputDialog.getText(
            self, "保存任务", "请输入任务名称:", QLineEdit.EchoMode.Normal, ""
        )
        ...
        if ok and task_name:
            try:
                ...
                self.log.append(
                    f"已保存任务 '{task_name}': {len(self.lrc_files)} 个文件"
                )
            except Exception as e:
    ...
    def clear_database_cache(self):
        ...
        try:
            self.audio_cache.clear_cache()
            QMessageBox.information(self, "清理完成", "数据库缓存已清空")
        except Exception as e:
            QMessageBox.warning(self, "清理失败", f"清理数据库缓存时出错：{str(e)}")
...
"""
            ...
            QMessageBox.warning(self, "获取统计失败", f"获取缓存统计时出错：{str(e)}")
